...
def _clear_embeddings_cache():
    ...
    runtime._get_embeddings_cached.cache_clear()
    runtime._last_embeddings_key_fp = None
    yield
    runtime._get_embeddings_cached.cache_clear()
    runtime._last_embeddings_key_fp = None
...
def test_langchain_runtime_get_embeddings_busts_cache_on_api_key_rotation(monkeypatch):
    """APIキーがローテーションされた場合、同じ model 引数でも新しいキーで再生成されること（#944）。"""
    from services import langchain_runtime as runtime

    with patch("services.langchain_runtime.OpenAIEmbeddings") as mock_cls:
        mock_cls.side_effect = lambda **kwargs: MagicMock(name="embeddings", **{"openai_api_key": kwargs["api_key"]})

        monkeypatch.setenv("OPENAI_API_KEY", "sk-old")
        a = runtime.get_embeddings("text-embedding-3-small")
        a_again = runtime.get_embeddings("text-embedding-3-small")
        assert a is a_again  # 同一キーなら同一インスタンスを再利用

        monkeypatch.setenv("OPENAI_API_KEY", "sk-new")
        b = runtime.get_embeddings("text-embedding-3-small")

        assert a is not b  # ローテーション後は別インスタンス
        assert mock_cls.call_args.kwargs["api_key"] == "sk-new"

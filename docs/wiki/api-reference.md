...
一覧系エンドポイントの `limit` は **最大100件**で頭打ちにします（既定値はエンドポイントごと）。
コントローラーでは `httpapi.LimitQuery(ctx, "limit", 既定値)` を使い、`httpapi.MaxListLimit` に
上限を集約しています。`IntQuery` を直接使うと上限が付かないため、一覧APIでは使わないでください。

...
- フロントエンドは httpOnly Cookie（`user_token` / `refresh_token`）で保持し、`middleware.ts` が期限2分前に自動リフレッシュする
  - リフレッシュが **401/403**（失効が確定）なら、セッションCookieを削除して作り直させる。残すとログイン画面とローディングを無限に往復する（#1519）
  - リフレッシュが **5xx・タイムアウト・通信断**（一時障害）なら Cookie は残す。通信が不安定なだけの利用者を毎回ログアウトさせない
  - どちらの場合も、**期限切れのアクセストークンは後段へ渡さない**（`X-User-*` を注入せず、後段へ転送する Cookie からも `user_id` / `user_token` を外す）。そのリクエストは未ログイン扱いで描画される
  - 企業ポータル（`company_user_token` / `company_refresh_token`）も同じ扱い

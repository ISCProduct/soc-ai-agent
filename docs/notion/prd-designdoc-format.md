...
Issue作成直後は空。`github-issue-to-backlog.yml` が Backlog 課題を作り、GitHub Issue のタイトルを `[SOCAIAGENT-N] ...` へ書き換えてから確定する。タイトルにプレフィックスが付いたのを確認して 3ページとも埋める。空欄のままだと Backlog と突き合わせられない。
...
| --- | --- |
| `下書き` | 書きかけ。まだ起票していない |
| `承認済み` | **起票した時点でここ。** 起票まで進んだなら内容は合意済み |
| `実装中` | 実装PRを開いた |
| `完了` | **main へマージされた時点。** |
| `破棄` | 作らないと決めた |

`レビュー中` は使わない。起票時に `レビュー中` を入れていたため、誰も次へ動かさず全件がそこで止まっていた。

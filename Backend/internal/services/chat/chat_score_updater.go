...
// reason は任意の説明文（軸スコアには混ぜない。証拠としてログ・履歴に残る）。
// 戻り値の bool は「有効な品質回答かどうか」を示す（進捗カウントに使用）。
func (s *ChatService) processChoiceAnswer(ctx context.Context, userID uint, sessionID, answer, reason string, history []models.ChatMessage, jobCategoryID uint) (bool, error) {
	...
	// 理由テキストは証拠として会話履歴に残し、軸スコアは選択肢位置のままにする。
	// 文章品質スコアと軸位置はスケールが違うため混ぜると、理由を書いた人ほど中央に寄る
	// （#1124 の飽和再発）／低選択が文章力で上がる、という判断不具合になる。
	if reason = strings.TrimSpace(reason); reason != "" {
		log.Printf("[Choice Answer] Reason kept as evidence only (len=%d); axis score=%d\n",
			len([]rune(reason)), result.Score)
	}
...
func (s *ChatService) inferCategoryFromQuestion(question string) string {
	// 実際に出題される質問文の語彙まで含める。
	// ここに載っていない語しか持たない質問は default(技術志向) に落ち、
	// 出題したカテゴリが永久に未評価のままループするため（#929）。
	// question_category_roundtrip_test.go がこの整合を強制する。
	//
	// 走査は valueobject.AllWeightCategories() の順で行うので、
	// 複数カテゴリに当たる語は「正典の並びで先に来た方」が勝つ。
	// 曖昧な語を足すときは、先行カテゴリを奪わないか確認すること。
	categoryKeywords := map[string][]string{
		"技術志向":       {"技術", "プログラミング", "コーディング", "アルゴリズム", "システム設計", "技術的"},
		"チームワーク志向":   {"協力", "協働", "連携", "メンバー", "共同", "チームで", "チームの"},
		"リーダーシップ志向":  {"リーダー", "指導", "率いる", "マネジメント", "方向性", "意思決定", "まとめた", "まとめ役", "主導", "リード", "巻き込"},
		"創造性志向":      {"創造", "アイデア", "発想", "革新", "イノベーション", "工夫", "改善"},
		"安定志向":       {"安定", "確実", "堅実", "リスク回避", "慎重", "腰を据え", "落ち着いた環境", "長く"},
		"成長志向":       {"成長", "キャリア", "昇進", "スキルアップ", "学習", "学ぶ", "学んだ", "新しく始め"},
		"ワークライフバランス": {"ワークライフ", "残業", "休日", "プライベート", "働き方", "時間の使い方", "時間配分"},
		"チャレンジ志向":    {"チャレンジ", "挑戦", "困難", "新しいこと", "未経験", "乗り越え", "プレッシャー", "切り替え"},
		"細部志向":       {"細部", "詳細", "正確", "精密", "丁寧", "計画", "期限"},
		"コミュニケーション力": {"コミュニケーション", "説明", "伝え", "対話", "話す", "プレゼン", "発表", "調整", "やり取り"},
	}

	questionLower := strings.ToLower(question)
	// map をそのまま range すると反復順がランダムになり、複数カテゴリの
	// キーワードに当たる質問で「同じ質問・同じ回答でもリクエストごとに
	// 採点先が変わる」という非決定性が生まれる。正典の並び順で走査して固定する（#929）。
	for _, c := range valueobject.AllWeightCategories() {
		for _, keyword := range categoryKeywords[string(c)] {
	...
	return string(valueobject.CategoryTechnical) // デフォルト
}

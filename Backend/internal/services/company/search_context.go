...
//
// ただし統合できるのは info と relations の2系統まで。tech(技術スタック)は
// 統合すると取得できなくなることを実測で確認した。
//
//	メルカリ  統合検索 技術キーワード 3/10 → 抽出結果 0件
//	          個別検索 技術キーワード 6/10 → 抽出結果 6件
//
// 統合検索の技術セクションは「公開情報からは確認できませんでした」で終わる。
...
func sharedSearchPrompt(companyName, websiteURL string) string {
	...
	return "日本の企業「" + companyName + "」" + siteHint + "について、公開情報から確認できる事実を調べてください。" +
		"次の2つは別のページに載っていることが多いので、どちらかに偏らず順に確認してください。\n\n" +

...
// web_search のほうが安定して埋まる（2026-09 計測 / #1338 時点のプロンプトとモデル）。
// 分母の9は CompanyInfoResult の主要9項目
// (説明・業種・所在地・URL・設立年・従業員数・主要事業・文化・勤務スタイル)。
//
...
// これが companyInfoHasGaps の穴判定に含まれている限り、結局 web_search は呼ばれる。
// 現状は「たまたま全部埋まれば Search を省ける」程度の位置づけで、情報量を
// 減らさないことを優先している。
//
// この経路は既定で無効（#1345）。有効化の条件は enrichGapsWithAI 側のコメントを見ること。
//
// 数値はプロンプト文面・抽出モデル・minWebsiteTextRunes に依存する。
// そのどれかを変えたら測り直すこと。
//
// JavaScript で本文を描画するサイトでは文字がほとんど取れない
// (トヨタで648字。同じく 2026-09 計測)。薄ければ呼び出し側が web_search へ戻す。

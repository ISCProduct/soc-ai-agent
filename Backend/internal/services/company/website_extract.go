...
//
// 企業情報の取得は web_search(OpenAI Responses API)に頼ってきたが、検索結果が
// 固定8,000トークン/call として課金されるため、これが企業検索まわりのコストの
// 大半を占めている(#1124)。公式サイトのURLが分かっているなら、そのページを
// 読めば同じ事実が手に入り、LLM には抽出だけさせればよい。
//
// すべてのサイトで通用する方法ではない。JavaScript で本文を描画するサイトでは
// ほとんど文字が取れないため、薄ければ呼び出し側が web_search へ戻す。

...
func SetupGoogleCalendarRoutes(api *echo.Group, calendarController *controllers.GoogleCalendarController, userSecret string) {
	// 認証不要（OAuth コールバック）
	api.GET("/google-calendar/callback", calendarController.ConnectCallback)

	// 認証必須エンドポイント
	cal := api.Group("/google-calendar", EchoUserAuth(userSecret))

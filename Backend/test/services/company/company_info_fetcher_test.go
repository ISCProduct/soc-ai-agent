...
func TestCompanyInfoFetcher_FetchAndSave_AISearchFallback(t *testing.T) {
	...
	client := openai.NewWithBaseURL(srv.URL, "gpt-4o-mini")
	fetcher := company.NewCompanyInfoFetcher(repo, client) // gBiz なし → AI Search Lite
	result, err := fetcher.FetchAndSave(context.Background(), 1, true)
	require.NoError(t, err)
...
func TestCompanyInfoFetcher_Acquire_InvalidJSON(t *testing.T) {
	...
	client := openai.NewWithBaseURL(srv.URL, "gpt-4o-mini")
	fetcher := company.NewCompanyInfoFetcher(nil, client)
...
func TestCompanyInfoFetcher_ConfirmAndSave(t *testing.T) {
	...
	fetcher := company.NewCompanyInfoFetcher(repo, nil)
	result, err := fetcher.ConfirmAndSave(1, &company.CompanyInfoResult{
...
func TestCompanyInfoFetcher_ConfirmAndSave_NilResult(t *testing.T) {
	fetcher := company.NewCompanyInfoFetcher(&mocks.CompanyRepositoryMock{}, nil)

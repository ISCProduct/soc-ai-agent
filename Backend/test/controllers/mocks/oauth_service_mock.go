...
func (m *OAuthServiceMock) HandleGoogleCallback(ctx context.Context, code string, tenantOrgID uint) (*services.AuthResponse, error) {
...
func (m *OAuthServiceMock) HandleGitHubCallback(ctx context.Context, code string, tenantOrgID uint) (*services.AuthResponse, error) {
	args := m.Called(ctx, code, tenantOrgID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
